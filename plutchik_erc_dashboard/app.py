...
from plutchik_erc_dashboard.utils.explainability import ExplainabilityEngine
from plutchik_erc_dashboard.utils.explainability_v2 import CaptumExplainer
from plutchik_erc_dashboard.utils.trainer import PluTchikTrainer
from plutchik_erc_dashboard.utils.llm_inference import NemotronClient


# ============== PLUTCHIK CONSTANTS ==============
from plutchik_erc_dashboard.utils.constants import PLUTCHIK, PRIMARY_EMOTIONS, EMOTION_NAMES, NUM_EMOTIONS
...
API_KEY = os.getenv("PLUTCHIK_API_KEY", "plutchik_secret_2026")

def call_api(endpoint: str, payload: dict, use_auth: bool = False):
...
with st.sidebar:
    ...
    analysis_mode = st.radio(
        "Analysis Protocol", 
        ["Single Utterance", "Conversation Arc", "Comparative Analysis", "Batch File Upload"],
        ...
        if analysis_mode == "Comparative Analysis":
            ...
            with st.spinner("📦 Processing batch file..."):
                ...
                if "text" not in df.columns:
                    ...
                    batch_req = {"items": [{"text": t, "scenario": scenario, "topic": topic} for t in df["text"].tolist()[:50]]} # Limit 50
...
if st.session_state.get("batch_results") is not None:
    ...
    fig_comp.update_layout(
        title="Divergent Emotional Arcs",
...
if st.session_state.get("prediction"):
    ...
    with tab_internals:
        ...
        with int_tab1:
            if pred["attribution_data"] is not None:
        ...
        with int_tab2:
            ...
            if "embedding_info" in pred and "all_token_embeddings" in pred["embedding_info"]:
                heatmap_data = pred["embedding_info"]["all_token_embeddings"]
        ...
        with int_tab3:
            ...
            if "embedding_info" in pred and "cls_embedding" in pred["embedding_info"] and emotion_centroids:
                ...
                for emotion_name, centroid in emotion_centroids.items():
                    sim = cosine_sim(cls_embedding, centroid)
                    similarities[emotion_name] = sim
