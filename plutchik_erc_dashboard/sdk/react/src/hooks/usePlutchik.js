...
export function usePlutchik({ 
  ...
  }, [baseUrl, sessionId, scenario, topic, apiKey]);

  /**
   * Submit a HITL correction back to the model
   */
  const submitCorrection = useCallback(async (text, predictedEmotion, correctedEmotion, confidence = 0) => {
    setLoading(true);
    setError(null);
    try {
